...
def GetListOfZipCodes(lat, long, range, zip_codes_table):
    '''range in km'''
    list_of_zip_codes = []
...
def main(csv_file, radius):
    ...
    for list in fire_data:
        [zip_codes_effected.add(result) for result in
         GetListOfZipCodes(float(list[0]), float(list[1]), radius, zip_codes_table)]
...
if __name__ == '__main__':
    ...
    # csv_file = 'D:\\CS Projects\\ca-fire-detection\\US ZIP codes-filtered.csv'
